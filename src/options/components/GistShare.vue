<template>
  <section class="p-8 border-b-2 border-gray-200">
    ...
    <div class="flex items-center space-x-2">
      ...
      <button
        ...
        :disabled="!token || syncAllGists"
        @click="syncGistHandler('all')"
      >
        <svg
          class="w-5 h-5"
          :class="{
            'animate-spin': syncAllGists
          }"
          viewBox="0 0 50 50"
          fill="currentColor"
          xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="py-4 pr-2 flex flex-wrap">
      <GistRecord
        ...
        :sync-gist-id="syncGistId"
        @edit-gist-record="editGistHandler(gistRecord)"
        @delete-gist-record="clearGistsRecordHandler(gistRecord.gistId)"
        @sync-gist="syncGistHandler(gistRecord)"
        @reset-sync-gist="syncGistId=''"
      />
    </div>
  </section>
  ...
  >
    <template #title>
      ...
      >
        {{ deleteGist === 'all' ? '删除所有 Gist' : `删除 ID 为 ${deleteGist} 的 Gist` }}
      </h2>
    </template>
    <template #msg>
      <p class="p-2 text-base text-center">
...
export default {
  ...
  setup(props) {
    ...
    const getFileContent = async (groups) => {
    ...
    const getSetGistResult = async (result) => {
      const fileContent = await getFileContent(result.groups);
      ...
      if (!result.gistId) {
        ...
        if (index === -1) {
          ...
          if (gist) {
            [filename] = Object.keys(gist.files)[0].split('.');
          } else {
            setMsg(false, '失败');
          }
        } else {
          filename = gistsRecord[index].filename;
        }

        const res = await updateGist(token.value, result.gistId, {
    ...
    // sync gist
    const syncGistId = ref('');
    const syncAllGists = ref(false);
    ...
    const syncGistHandler = async (result) => {
      ...
      if (result === 'all') {
        ...
        Promise.all(updatePromiseArr).then(() => {
          setMsg(true, '更新成功');
          syncAllGists.value = false;
          syncGistId.value = 'all';
        }).catch((err) => {
          syncAllGists.value = false;
    ...
    const clearGistsRecordHandler = (value) => {
      ...
      if (value === 'all') {
        ...
        deleteGist.value = value;
      }
    };
    const getDeleteGistsRecordResult = async (value) => {
      if (value) {
        if (deleteGist.value === 'all') {
